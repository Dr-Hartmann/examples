...
///
/// Каждое число представлено в виде связного списка, где цифры расположены в обратном порядке.
/// Например, число 342 представляется как [2, 4, 3].
///
/// # Arguments
///
/// * `l1` - Первое число в виде целого числа
/// * `l2` - Второе число в виде целого числа
///
/// # Returns
///
/// Возвращает результат сложения в виде связного списка (Box<ListNode>),
/// где цифры также расположены в обратном порядке.
///
/// # Examples
///
/// ```
/// // Представление 342 + 465 = 807
/// // 342 -> [2, 4, 3]
/// // 465 -> [5, 6, 4]
/// // 807 -> [7, 0, 8]
/// let result = add_two_numbers(342, 465);
/// ```
...
/// Преобразует связный список в целое число.
///
/// # Arguments
///
/// * `l` - Опциональный указатель на узел связанного списка, представляющий число
///
/// # Returns
///
/// Возвращает целое число, представленное связным списком.
/// Каждый узел содержит одну цифру, и порядок узлов соответствует разрядам числа.
///
/// # Examples
///
/// ```
/// // Представление [2, 4, 3] -> 342
