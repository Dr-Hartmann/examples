...
from .modelable import TrainableModel
from src.myutils import get_file_paths, move_files, IMG_TYPES


class YamlTrainableModel(TrainableModel):
    ...
    def _findall(self, root: ET.Element, name: str) -> Iterable[ET.Element]:
    ...
    def _unpack_move_join_imgs_and_ann(self, root: ET.Element, archives, tp, ann) -> ET.Element:
        for arch in tqdm(archives, desc="Распаковка zip-архивов..."):
            with ZipFile(arch, "r") as zip:
                zip.extractall(tp)
                ann_root = self._get_ann_root(tp / ann)
                if ann_root:
                    root = self._append(root, ann_root)
